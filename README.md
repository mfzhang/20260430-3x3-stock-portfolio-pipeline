...
**Status (2026-06-11, v2.3.15)**: The Trial #58 hyperparameters above are now **superseded**. A pre-registered Optuna re-run under the NLL architecture (60 trials, 5-fold stratified, SNDK-excluded) returned a mechanical **"Diverged"** verdict — 3 of 6 dimensions (lr, weight_decay, architecture) fall outside their pre-registered robust ranges vs Trial #58. New best config: Trial 52 (`large [128,64,32]`, `lr=1.56e-3`, `weight_decay=4.49e-5`), rank_corr 0.573 at N=5 — not directly comparable to the N=20 numbers above. Per the pre-registered consequence, v2.3.16 retrains production at the Trial 52 config; the table above remains the deployed baseline until then. Evidence: `results/stage1_v2315/best_trials.json`; verdict: Amendment 3 in `pre_registration_v2315_optuna_rerun.md`.
...
- **v2.3.15 re-ran the Stage 1 Optuna search under the NLL architecture** — the v2.3.7 search had optimized a Huber-loss MLP, making its hyperparameters architecturally stale. 60/60 trials completed (~135h). Pre-registered mechanical verdict: **Diverged** (3/6 dimensions outside robust ranges); Trial #58 superseded. Secondary checks: 33/60 trials above rank_corr 0.55 (broad plateau), 0/1500 NN trainings cap-bounded. v2.3.16 Stage 2 retrain at N=20 proceeds next. See Amendment 3 in `pre_registration_v2315_optuna_rerun.md`.
